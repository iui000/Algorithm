 ...
 * 逆序对
 *
 * 作为程序员的小Q，他的数列和其他人的不太一样，他有个2^n数。
 * 老板问了小Q一共 m次，每次给出一个整数, 要求小Q把这些数每分为2^（qi）一组，然后把每组进行翻转，小Q想知道每次操作后整个序列中的逆序对个数是多少呢？
 *
 * 例如:
...
public class inversionPair {
    ...
    private static void mergeSort(int[] arr, int left, int right, long[] reOrder, int index){
        if(left < right){
            ...
            而 reOrder[n] 求的是 [2,1] 中的元素 与 [4,3] 中的元素对比来说的逆序对，是两个数组元素互相形成的逆序对，
            而不是自己数组内部形成的逆序对
             */
            ...
            if(arr[mid] > arr[mid + 1]){
                merge(arr, left, right, mid, reOrder, index);
            }
        }
    }
    public static void merge(int[] arr, int left, int right, int mid, long[] reOrder, int index) {
        //mid 属于左边
        int len1 = mid - left + 1;
        int len2 = right - mid;
        ...
        */
        ...
        //然后把右半边剩余的数复制到结果中
        //想想为什么不用处理剩余的左半边呢？因为arr的左半边本来就保存有原来的左半边
