...
/**
 * 找到二叉树中 符合二叉搜索树 条件的最大拓扑结构的大小
 *
 * 注意和第7题的区别，第七题是找BST子树，而这道题是找BST拓扑结构，并不一定完全包含子树的所有节点
 */
public class Problem_08_BiggestBSTTopologyInTree {
	...
	public static int modifyMap(Node node, int nodeValue, Map<Node, Record> nodeRecordMap, boolean s) {

		if (node == null || (!nodeRecordMap.containsKey(node))) {
			return 0;
		}

		Record record = nodeRecordMap.get(node);
		if ((s && node.value > nodeValue) || ((!s) && node.value < nodeValue)) {
			nodeRecordMap.remove(node);
			return record.l + record.r + 1;
		} else {
			int minus = modifyMap(s ? node.right : node.left, nodeValue, nodeRecordMap, s);
			if (s) {
